...
export function MarkdownRenderer({
  ...
  const components = getComponents(onLinkClick);

  return (
    <div className="space-y-3">
      <Markdown
        remarkPlugins={[remarkGfm]}
        components={components as unknown as import("react-markdown").Components}
      >
        {children}
      </Markdown>
    </div>
...
function childrenTakeAllStringContents(element: unknown): string {
  ...
  const typedElement = element as { props?: { children?: unknown } };
    ...
    a: ({
      ...
      const handleClick = (e: React.MouseEvent) => {
        if (href && onLinkClick) {
          ...
          const handled = onLinkClick(href, text);
          if (handled) {
            e.preventDefault();
            e.stopPropagation();
            return;
